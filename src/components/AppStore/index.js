...
class AppStore extends Component {
  ...
  getActiveTabApps = () => {
    ...
    const searchResults = appsList.filter(
      eachResult => eachResult.category === activeTab,
    )
  ...
  render() {
    const {searchInput, appItemsList, activeTab} = this.state
    const searchResults = this.getActiveTabApps()
    const filteredAppsList = searchResults.filter(eachApp =>
          ...
          <ul className="list-container">
            {tabsList.map(eachTab => (
              <TabItem
                tabItem={eachTab}
                key={eachTab.tabId}
                onClickItemChange={this.onClickItemChange}
